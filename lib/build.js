...
module.exports = (str, opts) => {
  ...
  const postprocess = res => {
    ...
    return postcssrc()
      ...
      .then(result => {
        log('Writing CSS to %s ...', outputFile);

        fse.writeFileSync(outputFile, result.css);
      ...
      .then(result => {
        if (outputMapFile && result.map) {
          ...
          return fse.writeFile(outputMapFile, result.map).then(() => result);
        }
      ...
      .then(() => {
        ...
        return fse.writeFile(
          outputJSFile,
          `module.exports = { cssstats: require('./${dataFile}') }`
        );
      });
  };
